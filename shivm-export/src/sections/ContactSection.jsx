...
import { submitInquiry, getCategories } from '../services/api';
...
export default function ContactSection() {
  ...
  const [form, setForm] = useState({ name: '', phone: '', company: '', email: '', country: '', product: '', quantity: '', message: '' });

  useEffect(() => {
    getCategories()
      .then((res) => setApiCategories(res.data?.categories || res.data || []))
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await submitInquiry({
        ...
        quantity: form.quantity ? parseInt(form.quantity) || undefined : undefined,
      });
      setSubmitted(true);
    } catch (err) {
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <p className="text-white/50 text-lg max-w-2xl mx-auto">
            Ready to source premium girls' clothing? Fill the form below and our export team will respond within 24 hours.
          </p>
        </motion.div>
            ...
            <div className="glass rounded-3xl p-8">
              {!submitted ? (
                <>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid sm:grid-cols-2 gap-4">
                      {[
                        ...
                        <input
                          key={field.name}
                          type={field.type}
                          required={['name', 'phone'].includes(field.name)}
                    ...
                    <motion.button
                      type="submit"
                      disabled={loading}
                      whileHover={{ scale: loading ? 1 : 1.02, boxShadow: '0 20px 40px rgba(16,185,129,0.25)' }}
                      whileTap={{ scale: loading ? 1 : 0.98 }}
                      className="w-full py-4 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white font-bold text-base rounded-xl flex items-center justify-center gap-2 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
                    >
                      {loading ? (
                ...
                >
                  ...
                  <button
                    onClick={() => { setSubmitted(false); setForm({ name: '', phone: '', company: '', email: '', country: '', product: '', quantity: '', message: '' }); }}
